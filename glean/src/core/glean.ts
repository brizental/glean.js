...
class Glean {
  ...
  static initialize(
    ...
    Glean.dispatcher.flushInit(async () => {
      ...
      //
      // IMPORTANT!
      // Any pings we want to send upon initialization should be sent before this.
  ...
  static get platform(): Platform {
    if (!Glean.instance._platform) {
      throw new Error("Attempted to access platform specific APIs before Glean was initialized.");
    }
   ...
   * **Test-only API**
   *
   * Initialize Glean in testing mode.
   *
   * All platform APIs are mocked.
   *
   * @param applicationId The application ID (will be sanitized during initialization).
   * @param uploadEnabled Determines whether telemetry is enabled.
  ...
  static async testResetGlean(applicationId: string, uploadEnabled = true, config?: Configuration): Promise<void> {
    ...
    try {
      ...
      // Do nothing.
      // It is expected that this will error in case this is the first we initialize Glean.
      //
      // Before initialization we cannot access the databases.
    }

    // Re-Initialize Glean.
    Glean.testInitialize(applicationId, uploadEnabled, config);
